...
export default function EngineeringMethod() {
  const t  = useTranslations("engineering.method");
  const [active, setActive] = useState(0);
  const keys = ["read", "plan", "coordinate", "follow"] as const;
      ...
      <div className="relative z-10 max-w-[1440px] mx-auto px-6 lg:px-12">
        ...
        >
          <div className="flex items-center gap-4 mb-6">
            ...
            <p className="text-brand-copper text-[10px] tracking-[0.4em] uppercase font-semibold">{t("eyebrow")}</p>
          </div>
            ...
            {keys.map((k, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-lg lg:text-xl font-light tracking-wide flex-1">{t(`items.${k}.label`)}</span>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <p className="text-brand-copper text-[10px] tracking-[0.3em] uppercase font-semibold">{t(`items.${keys[active]}.label`)}</p>
                </div>

                <h3 className="text-2xl lg:text-3xl font-light text-white mb-5 leading-snug">{t(`items.${keys[active]}.title`)}</h3>
                <p className="text-brand-light/70 text-sm leading-relaxed">{t(`items.${keys[active]}.description`)}</p>
                ...
                <p className="text-white/30 text-[9px] tracking-widest mt-3 font-semibold">
                  {t("stage")} {String(active + 1).padStart(2, "0")} {t("of")} {String(keys.length).padStart(2, "0")}
                </p>
              </motion.div>
            </AnimatePresence>
          </div>
