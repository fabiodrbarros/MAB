...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: t("titleLine1"), robots: { index: false, follow: false } };
}
      ...
      <div className="relative z-10 max-w-2xl mx-auto text-center">
        ...
        <div className="pt-8 border-t border-white/10">
          ...
          <nav className="flex flex-wrap justify-center gap-x-6 gap-y-3 text-[11px] tracking-[0.2em] uppercase font-semibold">
            {shortcuts.map((s) => (
              <Link key={s.href} href={s.href} className="text-white/60 hover:text-brand-copper transition-colors">
                {tNav(s.key)}
              </Link>
            ))}
          </nav>
