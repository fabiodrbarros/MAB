...
export default async function ProjectDetailPage({ params }: { params: { id: string } }) {
  ...
  const hasImgs = project.images.length > 0;
      ...
      <section className="relative pt-32 pb-12 lg:pt-40 lg:pb-20 bg-white overflow-hidden">
        <div className="absolute inset-0 pointer-events-none opacity-50">
          <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <pattern id="prj-grid" width="40" height="40" patternUnits="userSpaceOnUse">
                <path d="M 40 0 L 0 0 0 40" fill="none" stroke="rgba(42,42,42,0.05)" strokeWidth="0.5"/>
              </pattern>
            </defs>
            <rect width="100%" height="100%" fill="url(#prj-grid)"/>
          </svg>
        </div>

        <div className="relative z-10 max-w-[1440px] mx-auto px-6 lg:px-12">
          <Link href="/projetos" className="inline-flex items-center gap-2 text-brand-grey hover:text-brand-copper text-[10px] tracking-[0.2em] uppercase font-semibold mb-8 transition-colors">
            ← Voltar à galeria
          </Link>
          ...
          {project.location && (
            <p className="text-brand-grey text-base lg:text-lg">{project.location}</p>
          )}
        </div>
      </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:row-span-2 md:col-span-1">
                <div className="relative aspect-[4/3] md:aspect-[3/4] bg-brand-offwhite overflow-hidden">
                  <Image src={project.images[0]} alt={project.title} fill className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" />
                </div>
              </div>
              {project.images.slice(1, 5).map((img, i) => (
                <div key={img} className="relative aspect-[4/3] bg-brand-offwhite overflow-hidden">
                  <Image src={img} alt={`${project.title} ${i+2}`} fill className="object-cover" sizes="(max-width: 768px) 100vw, 25vw" />
                </div>
              ))}
            </div>
          ) : (
          ...
          <div className="grid lg:grid-cols-3 gap-12 lg:gap-16">
            <div className="lg:col-span-2">
              ...
              <p className="text-brand-dark text-lg leading-relaxed whitespace-pre-line">
                {project.description || project.shortDescription || "Descrição a disponibilizar brevemente."}
              </p>
            </div>
            ...
            <aside className="lg:sticky lg:top-32 lg:self-start">
              <div className="relative border border-brand-copper/40 bg-brand-copper/[0.04] p-8">
                ...
                <dl className="space-y-4">
                  {[
                    ["Categoria",   project.categoryLabel],
